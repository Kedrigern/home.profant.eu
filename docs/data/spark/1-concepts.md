...
Basic concepts for working with OLAP (Online Analytical Processing, big data analytics).
...
- FULL [OUTER] JOIN: all rows from both tables, NULLs where no match. Also known as CROSS.
...
**Parquet files**: Highly efficient binary columnar data format for tables saved on disk.
A column is stored as one line, so you can read just one column and ignore others. It is strongly typed and compress.
...
Parquets are for storage on disk. Arrow is for in-memory (ram) data exchange. Both are columnar formats.
...
![delta table](./img/delta-lake.png)
